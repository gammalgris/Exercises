 ...
 */
public class FlatNetworkImpl implements Network {
    ...
    private void initializeNeurons(int... neuronsPerLayer) {
        ...
        for (int layer = FIRST_LAYER_INDEX; layer < neuronsPerLayer.length; layer++) {
            ...
            for (int count = 0; count < neuronCount; count++) {
                ...
                Neuron neuron = new ConcurrentNeuronImpl();
                Number randomBias = NetworkHelper.randomBias();
                setBias(neuron, randomBias);

                ActivationFunction activationFunction = ActivationFunctions.randomActivationFunction();
    ...
    public List<Synapse> synapses() {
        ...
        for (int layerIndex = FIRST_LAYER_INDEX; layerIndex < layers(); layerIndex++) {
            ...
            for (Neuron neuron : neuronsInLayer) {
                ...
                allSynapses.addAll(neuron.signalDestinations());
            }
        }

        allSynapses.add(outputSynapse());

        return Collections.unmodifiableList(allSynapses);
    }

    /**
     * The neurons are activated and send signals.
     */
    @Override
    ...
    /**
     * The neurons are deactivated and stop signalling.
     */
    @Override
    ...
    public Number send(Number input) {
        ...
        Number output = outputSignal.value();
